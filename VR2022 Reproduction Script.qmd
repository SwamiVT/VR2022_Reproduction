...
m.quad.mixed <- lmer(log(RT) ~ (1+Flanker|Participant_ID) +
                 Cue * Flanker +
                 Cue * poly(c.(Age), 2) +
                 Flanker * poly(c.(Age), 2) +
                 Sex + c.(Education) + c.(Trial),
               ant.mixed.corrected)
print(summary(m.quad.mixed), cor=F)
cbind(summary(m.quad.mixed)$coefficients, p.lmer(m.quad.mixed))
confint(m.quad.mixed, parm=c("Cue.ALERTING:poly(c.(Age), 2)2", "Cue.ORIENTING:poly(c.(Age), 2)2", "Flanker.EXECUTIVE:poly(c.(Age), 2)2"))
